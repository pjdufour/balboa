...
import nova.balboa.*;
import nova.library.xml.*;
import nova.library.graphics.textures.procedural.ProceduralTextureEquation;
import nova.library.settings.Settings;
import nova.library.utilities.*;
...
public class BalboaTexture
{
	...
	public static final boolean JOGL_ENABLED = false;
	
	private Balboa balboa;
	public int type;
	public String name;
	...
	public static BalboaTexture createFileTexture(Balboa balboa,String name,String filename)
	{
		BalboaTexture texture = new BalboaTexture();
		texture.balboa = balboa;
		texture.name = name;
		texture.type = TEXTURE_TYPE_FILE;
		texture.filename = filename;
		return texture;
	}
	public static BalboaTexture createProceduralTexture(Balboa balboa,String name,Node root)
	{
		BalboaTexture texture = new BalboaTexture();
		texture.balboa = balboa;
		texture.name = name;
	...
	public static BalboaTexture createPythonTexture(Balboa balboa,String name,Node root)
	{
		BalboaTexture texture = new BalboaTexture();
		texture.balboa = balboa;
		texture.name = name;
	...
	public void compile() throws GLException, IOException, ScriptException
	{
		...
		{
			if(JOGL_ENABLED) texture = TextureIO.newTexture(new File(filename),true);
		...
		else if(type==TEXTURE_TYPE_PROCEDURAL)
		{
			...
			if(JOGL_ENABLED) texture = TextureIO.newTexture(new ByteArrayInputStream(os.toByteArray()),true,"png");
			
			
			//Write To cache
			//Not necessary b/c can be read from memory
			String cache = balboa.getSettings().getString(Settings.CACHE_TEXTURES);
			if(!new File(cache).exists())
				new File(cache).mkdir();
			System.err.println("Writing "+name+" to "+cache+"\\"+name+".png");
			ImageIO.write(image,"png",new FileOutputStream(cache+"/"+name+".png"));
